  ...
  private static getPath(
    ...
    while (!finished) {
      ...
      if (pt.orientation === OrientationEnum.Left) {
        ...
        //   +---x---+
        if (
          ((pt.y - 1 >= 0 && facetResult.facetMap.get(pt.x, pt.y - 1) !== f.id) || // top exists and is a neighbour facet
        ...
        //   +---xnnnn
        if (
          ((pt.y + 1 < facetResult.height && facetResult.facetMap.get(pt.x, pt.y + 1) !== f.id) || // bottom exists and is a neighbour facet
        ...
        if (
          ((pt.x - 1 >= 0 && facetResult.facetMap.get(pt.x - 1, pt.y) !== f.id) || pt.x - 1 < 0) &&
          !xWall.get(pt.x, pt.y)
        ) {
        ...
        // check rotate to top
        if (
          ((pt.y - 1 >= 0 && facetResult.facetMap.get(pt.x, pt.y - 1) !== f.id) || pt.y - 1 < 0) &&
          !yWall.get(pt.x, pt.y)
        ) {
          // can place top _ wall at x,y
          if (debug) {
